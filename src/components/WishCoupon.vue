<template>
  <div class="relative w-full h-full overflow-y-auto bg-[#140411] p-4 flex flex-col justify-between hologram-grid pb-12">
    ...
    <div class="w-full text-center mt-3 mb-5 py-3 bg-black/50 backdrop-blur-md rounded-2xl border" :class="[themeClasses.border]">
      <span class="font-serif italic text-xs text-[#ffd27a] tracking-[0.1em] block mb-1">Redemption Center // 愿望兑换大厅</span>
      <h2 class="text-base font-bold text-white tracking-wide">老婆专属生日礼包 · 幸运指尖抽奖 🎁</h2>
    </div>
    ...
    <div class="w-full max-w-sm mx-auto mb-4 p-3 bg-black/75 border backdrop-blur-md rounded-xl text-center relative overflow-hidden" :class="[themeClasses.border]">
      ...
      <div class="flex justify-between items-center mb-1 text-[9px] font-sans text-gray-400">
        ...
        <span class="font-bold font-sans" :class="[themeClasses.text, { 'animate-pulse': isDrawing }]">● {{ isDrawing ? "DECELERATING // 欢快跳动中" : "READY // 待命" }}</span>
      </div>
    ...
    <div class="grid grid-cols-2 gap-2 sm:gap-3.5 w-full max-w-sm mx-auto mb-4 sm:mb-6 relative">
      <div v-for="(coupon, index) in coupons" :key="coupon.id" class="relative p-2 sm:p-4 rounded-xl sm:rounded-2xl bg-black/80 border transition-all duration-200 flex flex-col items-center justify-center text-center overflow-hidden h-24 sm:h-28 select-none border-white/5 bg-black/60" :class="[activeIndex === index ? themeClasses.cardActive : 'border-white/5 hover:border-white/10 bg-black/60', selectedCouponId === coupon.id && !isDrawing ? 'ring-2 ring-offset-2 ring-offset-black scale-102 z-10 ' + themeClasses.ringColor : '']" @click="selectCouponManually(coupon, index)" :id="'coupon_card_' + index">
        <!-- Scanner laser line sweep inside active hopping slot -->
        <div v-if="activeIndex === index" class="absolute inset-x-0 h-0.5 top-0 z-10 animate-[scannerSweep_0.8s_infinite_ease-in-out]" :class="[themeClasses.laserLine]"></div>
    ...
    <div class="w-full max-w-sm mx-auto mb-4 sm:mb-6 px-1">
      <button @click="startQuantumDraw" :disabled="isDrawing" class="w-full py-3 sm:py-4 rounded-xl sm:rounded-2xl font-sans font-black text-xs tracking-[0.1em] sm:tracking-[0.2em] text-white uppercase transition-all duration-300 border cursor-pointer select-none active:scale-98 disabled:pointer-events-none disabled:opacity-40" :class="[isDrawing ? 'bg-neutral-900 border-white/5 text-gray-500 shadow-none' : themeClasses.gradientBtn]" id="trigger_draw_btn">
        {{ isDrawing ? "正在为您抽取甜蜜记忆心愿..." : "💝 开启指尖抽奖魔法" }}
      </button>
    </div>

    <!-- Holographic Receipt Card Popup for Successful Draw / Redemption -->
    <div v-if="redeemedReceipt" class="fixed inset-0 bg-[#140411]/95 backdrop-blur-xl z-50 flex flex-col items-center justify-center p-4 animate-fade-in animate-duration-300">
      ...
      <div class="w-full max-w-xs sm:max-w-sm rounded-3xl bg-black/95 p-5 sm:p-6 relative flex flex-col border" :class="[themeClasses.receiptBorder]">
        ...
        <div class="mx-auto -mt-9 mb-4 px-4 py-1 rounded-full flex items-center gap-2 border shadow-sm" :class="[themeClasses.bgAccent, themeClasses.borderAccent, themeClasses.shadowAccent]">
          ...
          <span class="font-sans text-[10px] tracking-wider font-bold" :class="[themeClasses.text]">Redeemed // 已锁定特权 ✨</span>
        </div>

        <!-- Receipt title text -->
        <h4 class="text-center text-base font-bold text-white tracking-wider mb-1 mt-2">🎉 恭喜老婆中签！</h4>
        <p class="text-center font-sans text-[9px] text-[#ffd27a]/80 tracking-widest uppercase mb-5 font-medium">验证秘钥: {{ redeemedReceipt.validationCode }}</p>
        ...
        <div class="mt-5 flex flex-col gap-2">
          ...
          <template v-if="!isSummoned">
            <p class="text-[10px] text-gray-400 text-center leading-relaxed font-sans">💡 提示：本券已自动保存归档，随时可找老公兑付 💝</p>

            <button @click="closeReceipt" class="w-full py-2.5 mt-1 rounded-xl text-white font-bold uppercase tracking-widest text-xs active:scale-98 transition-all cursor-pointer text-center" :class="[themeClasses.gradientConfirm]" id="close_receipt_btn">放入专属回忆柜</button>
          </template>
          ...
          <template v-else>
            <div class="p-3.5 rounded-2xl bg-red-950/20 border border-red-500/30 backdrop-blur-md text-left space-y-2 relative overflow-hidden">
              ...
              <div class="flex justify-between items-center border-b border-red-500/20 pb-2">
                ...
                <span class="font-mono text-xs px-2 py-0.5 rounded border" :class="[elapsedSeconds >= 600 ? 'text-red-400 bg-red-950/80 border-red-500 animate-pulse' : 'text-white bg-red-900/40 border-red-500/30']">
                  {{ elapsedSeconds >= 600 ? "⚠️ 超时中" : formatTime(600 - elapsedSeconds) }}
                </span>
              </div>

              <p class="text-[11px] text-gray-300 leading-relaxed font-sans">
                <span class="text-[#ffd27a] font-bold">老公已被系统强制接单</span>
                ，正在光速赶来执行！请线下开始计时监督。
              </p>
              ...
              <div class="pt-1.5 border-t border-red-500/10 flex flex-col gap-1 text-[10px] text-gray-400 font-sans">
                ...
                <div class="flex justify-between items-center">
                  <span>当前累计应罚:</span>
                  <span class="text-red-400 font-black text-xs animate-bounce flex items-center gap-0.5">{{ elapsedMinutes }} 次亲亲 😘</span>
                </div>
              </div>

              <!-- 监工面板交互按钮 -->
              <div class="flex gap-2 pt-1">
                <button @click="completeSummon" class="flex-1 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-[10px] tracking-wider transition-colors active:scale-95 cursor-pointer text-center" id="complete_summon_btn">✅ 承兑完成 (放过他)</button>
                <button @click="cancelSummon" class="py-2 px-2.5 rounded-lg bg-neutral-800 hover:bg-neutral-700 text-gray-400 font-bold text-[10px] transition-colors active:scale-95 cursor-pointer text-center" id="cancel_summon_btn">取消</button>
              </div>
            </div>
          </template>
        </div>
    ...
    <div class="w-full text-center mt-6">
      <button @click="emit('next')" class="pointer-events-auto px-5 py-2 rounded-xl border border-[#ffd27a]/40 text-[10px] tracking-wider uppercase text-[#ffd27a] hover:bg-[#ffd27a]/20 text-center cursor-pointer transition-colors" id="rewind_gate_btn">前往压轴生日庆典 🎂 ➔</button>
    </div>
  </div>
</template>
...
function playSuccessChord() {
  try {
    ...
    freqs.forEach((f, idx) => {
      ...
      gainNode.gain.linearRampToValueAtTime(0.2, audioCtx!.currentTime + idx * 0.08 + 0.03);
...
function startQuantumDraw() {
  ...
  const laps = 5; // 轮转圈数：跑马灯在停下前完整旋转的圈数
  // 关键算法修正：将总步数加 1。
  // 算法原理解析：
  // 1. stepIndex 从 0 开始递增，表示已经执行完毕的动画步数。
  // 2. 动画终止的条件是：当 stepIndex 递增到 totalSteps 时，停止下一次 runTick 的调度。
  // 3. 因此，最后一次真正被执行的 runTick 对应的 stepIndex 值为 totalSteps - 1。
  // 4. 在原逻辑中，totalSteps = laps * hopSequence.length + targetSequenceIndex。
  //    导致最后一次执行的 activeIndex 对应 (totalSteps - 1) % len = targetSequenceIndex - 1，
  //    使视觉高亮位置刚好落在了目标奖品的前一个格子上，而数据弹窗却已经按 targetWinnerId（目标奖品）渲染。
  // 5. 通过在此处增加 1，使最后一次执行的 activeIndex 恰好对应 targetSequenceIndex，从而保证视觉高亮与弹窗奖品一致。
  const totalSteps = laps * hopSequence.length + targetSequenceIndex + 1;
